...
In XState, you can define a history state by setting the `type` property to `"history"` in the state configuration. Here's how you can model the payment form example using XState:
...
In this exercise, you'll model a subscription form with two subscription types: free and pro. The user can switch between the free and pro subscription types. When the user enters their name, they can go back to the subscription type they were using.
...
2. Add transitions between the states as follows:
   ...
   - From `name` to `subscription.hist` on the `go.back` event. \* Use the `hist` state to remember the last active state within the `subscription` state.
